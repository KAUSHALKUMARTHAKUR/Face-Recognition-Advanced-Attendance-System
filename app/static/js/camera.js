document.addEventListener("DOMContentLoaded", () => {
  ...
  function setupAttendanceSection(config) {
    ...
    async function startCamera() {
      try {
        ...
        previewCanvas.height = Math.round(
          (previewCanvas.width * (video.videoHeight || 480)) / (video.videoWidth || 640),
        )
    ...
    function startPreview() {
      ...
      previewTimer = setInterval(async () => {
        ...
        try {
          ...
          const res = await fetch("/api/liveness_preview", {
            method: "POST",
    ...
    async function markAttendance() {
      try {
        ...
        const payload = {
          student_id:
            (studentIdEl && studentIdEl.value) || (markBtn && markBtn.dataset && markBtn.dataset.studentId) || null,
  ...
  // Optional: helper to fetch attendance data and render somewhere
  async function getAttendance(filters = {}) {
    const params = new URLSearchParams(filters)
    const res = await fetch(`/api/get_attendance?${params.toString()}`, {
      method: "GET",
  ...
  // Attendance Camera (updated to /api/* endpoints)
  ...
  // Example usage for getAttendance (if you have corresponding UI):
  // const btn = document.getElementById('getAttendanceBtn');
  // if (btn) {
  //   btn.addEventListener('click', async () => {
  //     const data = await getAttendance({ course: 'CS101' });
